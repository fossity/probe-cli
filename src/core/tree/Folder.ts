...
export default class Folder extends Node {
  ...
  private statusLogic(): NodeStatus {
    ...
    if (this.children.some((child) => child.getStatus() === NodeStatus.IDENTIFIED))
      return NodeStatus.IDENTIFIED;
  ...
  public attachResults(component: any, path: string): boolean {
    ...
    const existComponent = this.components.some(
      (el) => el.purl === component.purl && el.version === component.version,
    );
  ...
  public getFiles(banned: FileExclusion = null): Array<any> {
  ...
  public getClonePath(paths: Record<string, number>): Node {
    const childrenClone = this.children
      .map((child) => child.getClonePath(paths))
